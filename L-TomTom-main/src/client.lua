...
-- Deze functie ontvangt het playerId van de NUI en zoekt de coördinaten
...
-- Deze functie ontvangt de postcode van de NUI en zet een waypoint op de kaart
...
function setWaypoint(postalCode)
    ...
    if postalCode_coords.x ~= 0.0 and postalCode_coords.y ~= 0.0 then
        ...
        -- Voeg de blip toe met de blauwe kleur (blip kleur ID 3)
        ...
        -- Optioneel: Voeg naam toe aan de blip
        --BeginTextCommandSetBlipName("STRING")
        --AddTextComponentString("Location")
        --EndTextCommandSetBlipName(blip)
